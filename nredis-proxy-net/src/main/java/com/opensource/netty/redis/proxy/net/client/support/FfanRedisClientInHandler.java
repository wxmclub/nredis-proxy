 ...
 *
 */
public class FfanRedisClientInHandler extends SimpleChannelInboundHandler<IRedisReply> {

	@Override
	protected void channelRead0(ChannelHandlerContext ctx, IRedisReply msg)
			throws Exception {
			IConnectionCallBack callBack = ctx.channel().attr(FfanRedisClientOutHandler.CALLBACK_KEY).get();
			if(callBack!=null){
