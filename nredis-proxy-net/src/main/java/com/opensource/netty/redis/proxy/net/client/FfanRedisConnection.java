 ...
 *
 */
public class FfanRedisConnection implements IConnection{
	
	private Logger logger = LoggerFactory.getLogger(FfanRedisConnection.class);
	 ...
	 */
	public FfanRedisConnection(FfanRedisClient ffanRedisClient) {
		super();
	...
	public void write(RedisCommand request,IConnectionCallBack connectionCallBack) {
		...
		this.channel.attr(FfanRedisClientOutHandler.CALLBACK_KEY).set(connectionCallBack);
	...
	public boolean open() {
		...
		try {
			...
			if (timeout <= 0) {
	            throw new FfanRedisProxyFrameworkException("Netty4Client init Error: timeout(" + timeout + ") <= 0 is forbid.",
